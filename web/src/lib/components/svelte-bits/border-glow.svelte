<script module lang="ts">
	...
	export function buildMeshGradients(colors: string[]): string[] {
		...
		for (let i = 0; i < 7; i++) {
			...
			out.push(`radial-gradient(at ${GRADIENT_POSITIONS[i]}, ${color} 0px, transparent 50%)`);
		}
...
<script lang="ts">
	...
	const borderBg = $derived(meshGradients.map((gradient) => `${gradient} border-box`));
	const fillBg = $derived(meshGradients.map((gradient) => `${gradient} padding-box`));
	...
	const borderMask = $derived(
		directional
			? `conic-gradient(from ${angleDeg} at center, black ${coneSpread}%, transparent ${
					coneSpread + 15
	...
	const edgeFillBoxShadow = $derived(
		`inset 0 0 18px color-mix(in oklab, ${colors[0]} ${Math.round(
			edgeFillOpacity * 100
		)}%, transparent), inset 0 0 34px color-mix(in oklab, ${colors[1]} ${Math.round(
			edgeFillOpacity * 40
		)}%, transparent)`
	);
</script>
...
<div
	...
	class="relative isolate border border-white/15 {className}"
	role="presentation"
